#!/usr/bin/env python3
"""Build Xinchang v2 from the WHO p.168 straight palmar hand."""
...
OUTPUT_PNG = HERE / "心常穴_正式評估版_v2_WHO-p168左手掌面.png"
OUTPUT_SVG = HERE / "心常穴_正式評估版_v2_WHO-p168左手掌面.svg"
OUTPUT_JSON = HERE / "心常穴_定位資料_v2.json"
...
# The p.168 palmar image is mirrored horizontally from the dorsal source.
# These lines remain inside the same middle finger. The transverse endpoints
# follow the local PIP/MCP direction rather than the page horizontal.
A_TOP = (389.5, 178.5)
A_BOTTOM = (396.0, 212.5)
E_TOP = (415.5, 173.5)
E_BOTTOM = (422.0, 207.5)
...
def centred_text(draw, xy, text, text_font, fill):
    draw.text(
        (p(xy[0]), p(xy[1])),
        text,
        font=text_font,
...
def draw_inset_annotations(draw):
    ...
    line(draw, inset_map(A_TOP), inset_map(E_TOP), MUTED, 1.5, (7, 5))
    line(draw, inset_map(A_BOTTOM), inset_map(E_BOTTOM), MUTED, 1.5, (7, 5))

    for name, top, bottom, colour, width in (
        ("A", A_TOP, A_BOTTOM, BLUE, 1.5),
        ("C", C_TOP, C_BOTTOM, BLUE, 1.5),
        ("D", D_TOP, D_BOTTOM, RED, 2.0),
        ("E", E_TOP, E_BOTTOM, BLUE, 1.5),
    ):
        line(draw, inset_map(top), inset_map(bottom), colour, width)
        label = inset_map(top)
        centred_text(
            draw,
            (label[0], label[1] - 19),
            name,
            font(14),
            VERMILLION if name == "D" else BLUE,
        )
