#!/usr/bin/env python3
"""Build Xinchang v4 from the WHO p.168 straight palmar hand."""
...
OUTPUT_PNG = HERE / "心常穴_正式評估版_v4_WHO-p168左手掌面.png"
OUTPUT_SVG = HERE / "心常穴_正式評估版_v4_WHO-p168左手掌面.svg"
OUTPUT_JSON = HERE / "心常穴_定位資料_v4.json"
...
def draw_inset_annotations(draw):
    ...
    line(draw, inset_map(A_TOP), inset_map(E_TOP), MUTED, 2.2, (8, 5))
    line(draw, inset_map(A_BOTTOM), inset_map(E_BOTTOM), MUTED, 2.2, (8, 5))

    for name, top, bottom, colour, width in (
        ("A", A_TOP, A_BOTTOM, BLUE, 2.5),
        ("C", C_TOP, C_BOTTOM, BLUE, 2.5),
        ("D", D_TOP, D_BOTTOM, RED, 3.2),
        ("E", E_TOP, E_BOTTOM, BLUE, 2.5),
    ):
