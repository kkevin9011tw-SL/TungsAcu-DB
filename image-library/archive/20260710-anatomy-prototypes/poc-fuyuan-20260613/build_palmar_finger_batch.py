...
SPEC = {
    ...
    "target_line": "D",
    "division_name": "四分點法",
    "point_fractions": (1 / 4, 1 / 2, 3 / 4),
    "point_count": "三穴",
    "reference": "原書復原穴圖",
}

OUTPUT_PNG = HERE / "復原穴_正式評估版_v1_WHO-p168左手掌面.png"
OUTPUT_SVG = HERE / "復原穴_正式評估版_v1_WHO-p168左手掌面.svg"
OUTPUT_JSON = HERE / "復原穴_定位資料_v1.json"
...
def draw_inset_annotations(draw):
    ...
    for fraction in (0, 1 / 4, 1 / 2, 3 / 4, 1):
...
def draw_labels(draw):
    ...
    centred_text(
        draw,
        (1050, 850),
        f'{SPEC["target_line"]} 線＝C–E 中點線｜取指節 1/4、1/2、3/4',
        font(15),
        MUTED,
    )
    draw.text(
