...
====================================================
【2026-07-12】互動人體地圖 Phase1 原型 + Phase2 擺點工具
====================================================

新增獨立前端層（不動現有 Streamlit）。設計/交接正本：
docs/superpowers/specs/2026-07-12-穴位人體地圖-design.md。

決策（為什麼）：
- 地圖走靜態/JS 前端（非 Streamlit）：動畫互動是 JS 主場、公開要載入快能被搜。
- 點紅點跳「側欄預覽」（穴名/穴號/取穴定位/主治，撈自 穴位表.csv），不新建靜態穴位頁。
- 密集區用 Voronoi 最近點感應＋SVG 縮放，不加深下鑽層級。
- 擺點一律「人工點一下擺點」，不走套皮的自動座標對映——後者需 registration/warp、
  常爆走（顥軒實測），地圖底圖固定、精度放寬，手動一次即夠準，藉此與製圖研究脫鉤。

產出（atlas/，已 push）：
- build_atlas.py→index.html：全身13部位標穴數→二二手背7穴示範。
- build_marker.py→marker.html：拖曳擺點、匯出 §6 JSON（換部位傳座標檔即可）。
- data/atlas-coords/二二部位.json：§6 座標格式範例（座標目測、待校正）。

卡住/待辦：
- production 底圖僅手背(02)/前臂(04)/小腿(14)三部位、無陰掌手掌；其餘待補。
- Phase1 那 7 顆紅點位置目測偏移——待顥軒用 marker 校正→回填→重跑 build_atlas.py。
- 後續：補各部位底圖與擺點、部署 Cloudflare Pages、Phase3 症狀反查亮穴。
