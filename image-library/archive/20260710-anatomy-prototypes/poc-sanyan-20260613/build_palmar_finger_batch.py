...
OUTPUT_PNG = HERE / "三眼穴_正式評估版_v1_WHO-p168左手掌面.png"
OUTPUT_SVG = HERE / "三眼穴_正式評估版_v1_WHO-p168左手掌面.svg"
OUTPUT_JSON = HERE / "三眼穴_定位資料_v1.json"
...
INSET_CY = 515.0
INSET_R = 272.0
INSET_CROP = (395.0, 161.0, 457.0, 223.0)

# Ring-finger PIP/MCP centres come from the WHO p.168 finger manifest.
# The calibrated middle-finger axes are translated at each joint, preserving
# their angle and spacing for this first ring-finger review.
A_TOP = (414.45, 176.43)
A_BOTTOM = (423.16, 210.80)
C_TOP = (422.81, 174.82)
C_BOTTOM = (431.98, 209.10)
E_TOP = (429.88, 173.46)
E_BOTTOM = (438.89, 207.77)
...
def draw_inset_annotations(draw):
    ...
    # Keep the division scale outside the target finger so it does not overlap
    # the A-E locating lines.
    offset = 145
    bracket_start = (
...
def draw_labels(draw):
    ...
    centred_text(
        draw,
        (1050, 850),
        f'{SPEC["target_line"]} 線＝A–C 中點線｜取指節上 1/3',
        font(15),
        MUTED,
    )
    draw.text(
        ...
        json.dumps(
            {
                ...
                "construction": {
                    "geometry": "ring-finger A line measured from user correction; C/E retained from WHO PIP/MCP geometry",
                    ...
                    "point_placement": "palmar B line on radial side",
                },
